...
`junit-platform-launcher`: Gradle 9 + JUnit 5 조합에서 런타임 런처를 명시적으로 요구함.
...
**`WorklogWriter`가 init 없이도 파일을 만드는 이유:**
`WorklogWriter`는 파일 append 책임만 가짐. "init 여부 확인"은 CLI 레이어(`requireInitialized`)의 역할. 레이어별 단일 책임 원칙.
