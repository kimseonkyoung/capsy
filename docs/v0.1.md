...
`junit-platform-launcher`: Gradle 버전과 환경에 따라 런타임 런처를 명시적으로 선언해야 할 수 있음. Gradle 9 + JUnit 5.11 조합에서 누락 시 테스트가 실행되지 않는 경우가 있어 명시적으로 추가함.
...
`{{WORKLOG_SNIPPET}}`은 나중에 LLM에 worklog를 주입할 때 사용할 자리.
...
**레이어별 책임 분리:**
`WorklogWriter`는 "주어진 경로에 append한다"는 책임만 가진다. `.capsy`가 존재하는지 확인하는 것은 CLI 레이어(`CapsyApp.requireInitialized()`)의 역할이다. 이 구분 덕분에 `WorklogWriter`를 테스트할 때 init 상태와 무관하게 `@TempDir`만으로 독립적인 검증이 가능하다.
